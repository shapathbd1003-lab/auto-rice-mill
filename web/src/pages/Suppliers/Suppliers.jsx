...
export default function Suppliers() {
  ...
  const openAdd = () => {
    setEditRow(null);
    setForm({ code: '', name: '', name_bn: '', phone: '', address: '', opening_balance: 0 });
    setFormOpen(true);
  };
  ...
  const FORM_FIELDS = [
    { key: 'code',    label: t('common.code'),        xs: 6 },
    { key: 'name',    label: t('common.name'),        xs: 6 },
    { key: 'name_bn', label: t('common.nameBangla'),  xs: 6 },
    { key: 'phone',   label: t('common.phone'),       xs: 6 },
    { key: 'address', label: t('common.address'),     xs: 12 },
  ];
              ...
              <CardContent sx={{ pb: 0 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <Box sx={{ textAlign: 'right' }}>
                    {row.balance > 0
                      ? <Chip label={`${t('common.due')} ৳${Number(row.balance).toLocaleString()}`} color="warning" size="small" />
                      : <Chip label={t('common.clear')} color="success" size="small" variant="outlined" />}
                  </Box>
                </Box>
              </CardContent>
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell align="right">{t('supplier.ledger')}</TableCell>
                ...
                : rows.map((row) => (
                  <TableRow key={row.id} hover>
                    ...
                    <TableCell align="right">
                      {row.balance > 0
                        ? <Chip label={`৳ ${Number(row.balance).toLocaleString()}`} color="warning" size="small" />
      ...
      <Dialog open={formOpen} onClose={() => setFormOpen(false)} maxWidth="sm" fullWidth fullScreen={isMobile}>
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            {!editRow && (
              <Grid item xs={6}>
                <TextField fullWidth size="small" label={t('common.openingBalance')} type="number"
